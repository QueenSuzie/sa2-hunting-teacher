...
namespace sa2_hunting_teacher.DropdownControls
{
    ...
    public class GroupedComboBox : ComboBox, IComparer
    {
        ...
        private static class NativeMethods
        {
            ...
            [DllImport("user32")]
            public static extern int GetWindowRect(nint hwnd, ref Rectangle lpRect);

            [DllImport("user32")]
            public static extern void SetWindowPos(nint hwnd, int hWndInsertAfter, int X, int Y, int cx, int cy, int wFlags);
                ...
                if (_hwndDropDown == nint.Zero)
                {
                    ...
                    try
                    {
                        ...
                        if (r.Top > cBounds.Top)
                        {
                            ...
                            NativeMethods.SendMessage(m.LParam, NativeMethods.LB_SETTOPINDEX, SelectedIndex, nint.Zero);

                            // repaint the dropdown
                            NativeMethods.RedrawWindow(m.LParam, nint.Zero, nint.Zero, RedrawWindowFlags.Erase | RedrawWindowFlags.Frame | RedrawWindowFlags.Invalidate | RedrawWindowFlags.AllChildren);
                        }
                    }
                    catch { }
                }
